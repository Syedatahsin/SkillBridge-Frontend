...
export default function TutorListSection() {
  ...
  return (
    <section className="py-20 px-4 max-w-7xl mx-auto text-white relative">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center min-h-[450px] gap-6">
          ...
          <p className="text-gray-400 font-bold uppercase text-[10px] tracking-[0.3em] animate-pulse">
            Curating top talent...
          </p>
        </div>
      ) : tutors.length === 0 ? (
        <div className="text-center py-32 bg-white/[0.02] border border-dashed border-white/10 rounded-[3rem] backdrop-blur-sm">
          <GraduationCap className="size-16 mx-auto text-gray-700 mb-4" />
          <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">No tutors found at the moment.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {tutors.map((tutor: any) => (
            <div 
              key={`tutor-${tutor.id}`} 
              className="group relative bg-[#0A0A0B] border border-white/10 p-8 rounded-[2.5rem] hover:border-purple-500/40 transition-all duration-500 flex flex-col justify-between overflow-hidden"
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-8">
                  <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-purple-500/20 to-fuchsia-500/20 flex items-center justify-center border border-white/10 group-hover:scale-110 transition-transform duration-500">
                ...
                <p className="text-sm text-gray-400 mt-4 line-clamp-3 leading-relaxed font-medium">
                ...
                <div className="flex flex-wrap gap-2 mt-6">
                   {tutor.categories?.slice(0, 3).map((c: any) => (
                     <span 
                        ...
                        className="text-[9px] uppercase font-black tracking-widest px-3 py-1.5 bg-white/5 border border-white/5 rounded-full text-purple-300 group-hover:bg-purple-500/10 group-hover:border-purple-500/20 transition-all"
                     >
                        {c.category?.name}
                     </span>
                   ))}
              ...
              <div className="mt-10 relative z-10">
                <Link href={`/tutorsingleprofile/${tutor.id}`}>
                  <Button className="w-full h-14 bg-white text-black hover:bg-gradient-to-r hover:from-purple-600 hover:to-fuchsia-600 hover:text-white transition-all duration-500 font-black uppercase text-[10px] tracking-[0.2em] rounded-2xl group-hover:shadow-[0_10px_30px_rgba(147,51,234,0.3)]">
      ...
      {!loading && tutors.length > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-center gap-8 mt-24">
          <div className="flex items-center p-2 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-xl">
            <Button
              variant="ghost"
              disabled={page <= 1}
              onClick={() => setPage((p) => p - 1)}
              className="hover:bg-white/10 text-white disabled:opacity-20 rounded-xl"
            >
              Prev
            </Button>

            <div className="flex gap-1 px-4 border-l border-r border-white/10">
